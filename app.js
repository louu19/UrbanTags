...
function initLanding() {
    ...
    function loadRandomSatellite() {
        ...
        google.maps.event.addListenerOnce(map, "tilesloaded", () => {
            if (container.querySelectorAll("img").length < 6) loadRandomSatellite();
        });
    }
    loadRandomSatellite();
...
function openTagUI() {
    ...
    canvas.onmousedown = e => {
        draw = true;
        currentPath = { color: ctx.fillStyle, size: document.getElementById("sizePicker").value, points: [{ x: e.offsetX, y: e.offsetY }] };
    };
    ...
    document.querySelectorAll("#colorPalette span").forEach(dot => { dot.style.background = dot.dataset.color; dot.onclick = () => ctx.fillStyle = dot.dataset.color; });
...
function drawNeutral(canvas, paths) {
    ...
    paths.forEach(p => {
        ...
        c.lineJoin = "round";
        c.beginPath();
        p.points.forEach((pt, i) => { if (i === 0) c.moveTo(pt.x, pt.y); else c.lineTo(pt.x, pt.y); });
        c.stroke();
    });
}
...
async function saveTag() {
    ...
    try {
        await addDoc(collection(db, "graffiti"), tagData);
...
// ---------------- ARCHIVE ----------------
async function loadArchiveMarkers() {
    ...
    const querySnapshot = await getDocs(collection(db, "graffiti"));
    querySnapshot.forEach(docSnap => {
        ...
        const tooltip = new google.maps.InfoWindow({
            content: `<div class="graffiti-tooltip">
                        <strong>Graffito</strong><br>
                        <img src="${tag.neutral}" alt="Graffito">
                        <small>${new Date(tag.time).toLocaleString()}</small>
                      </div>`
        });
        ...
        marker.addListener("click", () => {
            ...
            container.onclick = () => container.classList.add("d-none");
        });
    });
}
